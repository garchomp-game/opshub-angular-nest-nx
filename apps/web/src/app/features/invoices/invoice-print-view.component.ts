  ...
  template: `
    <div class="min-h-screen bg-base-200/50 flex flex-col pt-6 pb-12 print:bg-white print:p-0 print:m-0">
      @if (invoicesService.isLoading()) {
        ...
        <!-- A4 Print Container -->
        <div class="bg-white mx-auto print-content shadow-sm print:shadow-none border border-base-300 print:border-none" data-testid="print-content">
            ...
            <tbody class="divide-y divide-gray-200 border-b-2 border-gray-900">
              @for (item of inv.items; track item.id) {
                <tr data-testid="print-item-row" class="break-inside-avoid">
                  ...
                  <td class="py-4 px-4 text-right text-gray-800 font-mono">{{ item.quantity | number }}</td>
                  <td class="py-4 px-4 text-right text-gray-800 font-mono">¥{{ item.unitPrice | number }}</td>
